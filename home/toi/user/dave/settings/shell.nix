{ lib, pkgs, ... }:
{
  # Common session variables for shells
  sessionVariables = {
  ...
  # Shared shell functions (POSIX/bash-compatible where possible)
  shellFunctions = ''
    ...
    vld() { if [ -d "/var/local/data" ]; then cd /var/local/data; else echo "No /var/local/data"; return 1; fi }
    vldb() { if [ -d "/var/local/db" ]; then cd /var/local/db; else echo "No /var/local/db"; return 1; fi }
    vlds() { if [ -d "/var/local/data/_system" ]; then cd /var/local/data/_system; else echo "No /var/local/data/_system"; return 1; fi }

    wifi_scan() { if command -v nmcli >/dev/null 2>&1; then nmcli device wifi rescan && nmcli device wifi list; else echo "nmcli not available"; return 1; fi }
    derp() { if command -v curl >/dev/null 2>&1; then curl "https://cht.sh/$1"; else echo "curl not available"; return 1; fi }
    weather() { if command -v curl >/dev/null 2>&1; then curl -sSL "https://wttr.in?F"; else echo "curl not available"; return 1; fi }
    grep() { command grep --color=auto "$@"; }
    ports() { if command -v netstat >/dev/null 2>&1; then netstat -tulanp "$@"; else echo "netstat not available"; return 1; fi }
    tree() { if command -v tree >/dev/null 2>&1; then tree -Cs "$@"; else echo "tree not available"; return 1; fi }
    ...
    copyfile() {
      ...
      if command -v wl-copy >/dev/null 2>&1; then
        ...
        echo "no clipboard utility (wl-copy/xclip/pbcopy) available"
        return 1
      fi
    }
