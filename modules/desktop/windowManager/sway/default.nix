  ...
  config = mkIf (config.host.home.feature.gui.enable && displayServer == "wayland" && windowManager = "sway") {
    ...
    wayland.windowManager.sway = {
      ...
      startup = [
        ...
        { command = "autotiling"; always = true; }                                                                      # Auto Tile H/V
        { command = "dunst"; always = false; }                                                                          # Notification Manager
        { command = "opensnitch-ui --background"; always = false; }                                                     # Firewall
        { command = "ferdium --ozone-platform=wayland --enable-features-WaylandWindowDecorations"; always = false; }    # IM
        #{ command = "flameshot"; always = false; }                                                                     # Screenshot
        { command = "nextcloud --background"; always = false; }                                                         # Nextcloud Client
        { command = "opensnitch-ui"; always = false; }                                                                  # Firewall
        { command = "swayidle -w timeout 600 'swaylock -f -e -l -L  -c 000000' timeout 1800 'swaymsg ''output * dpms off'' resume 'swaymsg ''output * dpms on'' timeout 2700 'systemctl suspend' before-sleep 'swaylock -f -e -l -L -c 000000'"; always = false; }
        { command = "way-displays"; always = false; }                                                                   # Displays
        { command = "sleep 5; way-displays -c ~/.config/waydisplays/cfg.yaml -s ORDER DP-3 DP-2 HDMI-A-1" ; always = false; }
      ];
      terminal = "kitty";
