...
let
  ...
  sway-notification-center-latest = pkgs.swaynotificationcenter.overrideAttrs (oldAttrs: {
    ...
    buildInputs = oldAttrs.buildInputs ++ [ pkgs.wayland-scanner ];
  });
in
  with lib;
{
  ...
  config = mkIf cfg.enable {
    home = {
      packages = with pkgs;
        [
          sway-notification-center-latest
        ];
    };
